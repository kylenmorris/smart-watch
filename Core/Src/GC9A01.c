...
{
  	LCD_CS_0; 											// select disp
	LCD_DC_1; 											// data mode
	dma_spi_fl1=0;  									// our dma flag
	LL_DMA_DisableChannel(DMA_NO, DMA_CHANNEL);			// disable dma 
	LL_DMA_SetDataLength(DMA_NO, DMA_CHANNEL, size);	// set data length
	LL_DMA_EnableIT_TC(DMA_NO, DMA_CHANNEL);			// transfer complete interrupt enable
	LL_DMA_ClearFlag_TC4(DMA_NO);						// clear transfer complete flag
	LL_DMA_ClearFlag_TE4(DMA_NO);						// clear transfer error flag
	LL_DMA_ConfigAddresses(	DMA_NO, DMA_CHANNEL, 																	\
													(uint32_t)pbuff, 																				\
													LL_SPI_DMA_GetRegAddr(SPI_NO),														\
													LL_DMA_GetDataTransferDirection(DMA_NO, DMA_CHANNEL)	\
												);	
	LL_DMA_EnableChannel(DMA_NO, DMA_CHANNEL);
